...
export default function CDMusic() {
    ...
    return (
        <div className='flex items-center  gap-12'>
            {isTurnOnMusic &&
                <audio
                    src={dataMusic[isOrderMusic].url}
                    autoPlay
                    loop
                />
            }
            {
                changeMusic ? (
                    <View className=' bg-yellow-200  relative'
                        initial={{ x: 100 }}
                        animate={{ x: 0 }}
                        exit={{ x: 100 }}
                    ...
                    >
                        <div className='p-2 rounded-lg  backdrop-blur-[2rem] shadow-sd-primary border border-cl-btn-light-bg-primary absolute bottom-0 w-[12rem] right-0'>
                            <div className='h-[10rem] rounded-lg overflow-hidden border border-cl-btn-light-bg-primary'>
                                <Image src={dataMusic[isOrderMusic].thumbnail} width={800} height={800} className=' w-full h-full object-cover' alt={dataMusic[isOrderMusic].name} />
                            </div>
