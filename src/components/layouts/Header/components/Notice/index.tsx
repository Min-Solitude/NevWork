...
export default function Notice() {
    ...
    return (
        <div className='flex justify-center relative items-center min-w-[2rem]'>
            ...
            {
                show && (
                    <View className='absolute -bottom-[16rem] h-[15rem] flex justify-center items-center right-0 bg-bg-black-90 text-white shadow-sd-primary w-[20rem] p-4 rounded-lg'
                        initial={{ opacity: 0 }}
                        animate={{ opacity: 1 }}
                        transition={{ duration: 0.25 }}
                    >
                        {
                            loading ? <Loading kind='animated' /> : (
                                <div className=' w-full h-full'>
                                    <div className='flex w-full gap-4'>
                                        <Button className={`flex-1 text-base rounded-lg py-2 font-semibold ${isStatus === 'all' ? 'bg-cl-yellow-dark ' : 'bg-cl-btn-dark-bg-primary'}`}
                                            onClick={() => setIsStatus('all')}
                                        >Tất cả</Button>
                                        <Button className={`flex-1 text-base rounded-lg py-2 font-semibold ${isStatus === 'unread' ? 'bg-cl-yellow-dark ' : 'bg-cl-btn-dark-bg-primary'}`}
                                    ...
                                    <div className='text-base mt-4 w-full flex flex-col gap-4 hidden-scrollbar h-[9.5rem] overflow-y-scroll'>
                                        {
                                            (notice && notice?.length > 0) ? notice.map((item, index) => (
                                                <div key={index} className='bg-cl-btn-dark-bg-primary flex-col w-full items-start cursor-pointer hover:brightness-75 active:scale-95 duration-150  rounded-lg px-4 py-2 relative'
                                                    onClick={() => {
                                                        {
                                                ...
                                                >
                                                    <h1 className='font-semibold text-cl-yellow-dark line-clamp-1'>{item.title}</h1>
                                                    ...
                                                    <div className='flex w-full justify-between  mt-2 items-center'>
                                                        ...
                                                        <span className='text-xs font-semibold mb-1'>
                                                            {
                                                                item.status === 'unread' ? (
                                                                    <span className=' bg-red-500 py-1 px-2 rounded-full'>Chưa đọc</span>
                                                                ) : (
                                                                    <span className=' bg-green-500 py-1 px-2 rounded-full'>Đã đọc</span>
                                                                )
                                                            }
                                                        </span>
                                                    </div>
